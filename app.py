...
def get_robot_viewer_html(robot_name, command=None, kata_name=None):
    ...
    # This is the simplest, most reliable approach: importmap with a known working CDN
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <script type="module">
            ...
            const IS_KATA = IS_KATA;
            
            const robot = new THREE.Group();
            
            // Torso
            const torsoGeo = new THREE.BoxGeometry(0.9, 1.0, 0.6);
            const torsoMat = new THREE.MeshStandardMaterial({ color: KIMONO, roughness: 0.3, metalness: 0.7 });
            ...
            if (IS_KATA) {
                const beltGeo = new THREE.CylinderGeometry(0.55, 0.55, 0.12, 16);
                const beltMat = new THREE.MeshStandardMaterial({ color: BELT, roughness: 0.3, metalness: 0.2 });
            ...
            if (IS_KATA) {
                const headbandGeo = new THREE.TorusGeometry(0.28, 0.04, 8, 16);
                const headbandMat = new THREE.MeshStandardMaterial({ color: HEADBAND, roughness: 0.4, metalness: 0.3 });
            ...
            const shoulderMat = new THREE.MeshStandardMaterial({ color: KIMONO, roughness: 0.4, metalness: 0.6 });
            ...
            const upperArmMat = new THREE.MeshStandardMaterial({ color: KIMONO, roughness: 0.3, metalness: 0.7 });
            const lowerArmMat = new THREE.MeshStandardMaterial({ color: KIMONO, roughness: 0.4, metalness: 0.6 });
            ...
            const upperLegMat = new THREE.MeshStandardMaterial({ color: KIMONO, roughness: 0.5, metalness: 0.4 });
            const lowerLegMat = new THREE.MeshStandardMaterial({ color: KIMONO, roughness: 0.5, metalness: 0.3 });
            ...
            function updateKata(delta) {
                ...
                const action = kataAction; if (!action) return;
                const type = action[0]; const duration = action[1];
                if (type === 'idle') {
